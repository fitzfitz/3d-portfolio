...
**Asset route:** Blender 5.1.2 is installed at `/Users/fitzgeral/Applications/Blender.app` and headless `bpy` scripting is verified working (`Blender --background --python <script>`). Phase 3 models are generated by scripted Blender (mesh + modifiers for organic detail), exported GLB, then run through `npm run assets:optimize`. Generator scripts live in `scripts/blender/` and are committed — assets stay reproducible.

| # | Feature | Approach sketch |
|---|---------|-----------------|
| 3.1 | **NPC traffic** | 4–6 tiny cargo ships (bpy-generated low-poly hulls with emissive window/nav-light materials, one GLB, instanced) flying catmull-rom spline loops between planets; blinking nav lights (emissive pulse); slight avoidance bank when player within radius |
| 3.2 | **Space creature** | One large slow jellyfish/whale: bpy-generated body (metaballs/subsurf + displace for organic silhouette) + tentacle tubes; in-app vertex-shader sine deformation for undulation; additive fresnel glow; appears on a long loop so sightings feel rare |
